...
public partial class UserDefinedPlotView : UserControl
{
    ...
    private readonly double[] _ybuffer = new double[buffercount]; // 고정된 100 포인트 버퍼
    private readonly double[] _xbuffer = Enumerable.Range(0, buffercount).Select(i => (double)i).ToArray();
    private readonly double[] _zfill = Enumerable.Range(0, buffercount).Select(i => (double)0).ToArray();
    ...
    private void RefreshPlot()
    {
        ...
        var plot = UserDefindPlot.Plot.Add.FillY(_xbuffer, _zfill, _ybuffer);
    ...
    {
        // 왼쪽으로 밀기
        for (int i = 0; i < _ybuffer.Length - 1; i++)
            _ybuffer[i] = _ybuffer[i + 1];
